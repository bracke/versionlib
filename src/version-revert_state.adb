...
package body Version.Revert_State is
   use Version.Objects;
   ...
   begin
      return Version.Files.Join
        (Version.Repository.Git_Dir (Repo), "VERSION_REVERT");
   end State_Path;
   ...
   begin
      if Text = "symbolic" then
         ...
         raise Ada.IO_Exceptions.Data_Error with
           "malformed revert state: head kind";
      end if;
   end Kind_Value;
   ...
   begin
      ...
      if Commits.Is_Empty then
         raise Ada.IO_Exceptions.Data_Error with
           "malformed revert state: commits";
      end if;
      ...
      if Next_Index > Natural (Commits.Length) then
         raise Ada.IO_Exceptions.Data_Error with
           "malformed revert state: next index";
      end if;

      if Mainline > 0 and then Mainline > 16 then
         raise Ada.IO_Exceptions.Data_Error with
           "malformed revert state: mainline";
      end if;
      ...
      if Paused then
         ...
         then
            raise Ada.IO_Exceptions.Data_Error with
              "malformed revert state: current commit";
         end if;
      elsif Current_Commit'Length /= 0 then
         raise Ada.IO_Exceptions.Data_Error with
           "malformed revert state: current commit without pause";
      end if;
   ...
   begin
      if not Ada.Directories.Exists (Path) then
         raise Ada.IO_Exceptions.Data_Error with "no revert in progress";
      elsif not Version.Files.Is_Ordinary_File (Path) then
         raise Ada.IO_Exceptions.Data_Error with
           "malformed revert state: not a regular file";
      end if;

      Ada.Text_IO.Open (File, Ada.Text_IO.In_File, Version.Files.To_Native_Path (Path));

      if Ada.Text_IO.Get_Line (File) /= "version 1" then
         raise Ada.IO_Exceptions.Data_Error with "malformed revert state: version";
      end if;
      ...
      begin
         if Paused_Text = "yes" then
            ...
            raise Ada.IO_Exceptions.Data_Error with "malformed revert state: paused";
         end if;
      end;
      ...
      if S.Next_Index_Value > Total then
         raise Ada.IO_Exceptions.Data_Error with "malformed revert state: next index";
      end if;
      ...
      if not Ada.Text_IO.End_Of_File (File) then
         raise Ada.IO_Exceptions.Data_Error with "malformed revert state: trailing data";
      end if;
   ...
   exception
      when Ada.IO_Exceptions.End_Error =>
         ...
         raise Ada.IO_Exceptions.Data_Error with
           "malformed revert state: truncated";
      when others =>
         if Ada.Text_IO.Is_Open (File) then
            Ada.Text_IO.Close (File);
         end if;
