...
package body Version.Revert is
   ...
   --  Forward declaration: the clean-tree check needs each commit's revert
   --  parent to know which paths the revert would touch.

   --  An auto-committing revert (git's default) needs a wholly clean tree so
   --  it can commit; with --no-commit git is lenient and only refuses when a
   --  path the revert would change is also modified in the index or working
   --  tree (or is untracked and would be overwritten). A dirty file the revert
   --  leaves alone is then fine.
      ...
      for I in Revisions.First_Index .. Revisions.Last_Index loop
         declare
            ...
            Parent : constant Version.Objects.Hex_Object_Id :=
              Parent_For_Revert (Repo, Commit, Mainline);
